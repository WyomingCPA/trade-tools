...
class Cryptocurrency extends Model
{
    ...
    protected $appends = ['rsi_hour', 'rsi_day', 'ema_hour', 'ema_day', 'cci_hour', 'cci_day', 'pools_range'];
    ...
    {
        $candles = Candle::where('tools_id', '=', $this->id)->where('tools_type', '=', 'coins')->where('interval', '=', '1d')
            ->where('created_at', '>=', Carbon::now()->subMonths(7)->startOfDay())->orderBy('time', 'asc')->get();
    ...
    {
        $models = Candle::where('tools_id', '=', $this->id)->where('tools_type', '=', 'coins')
            ->where('interval', '=', '1d')->where('time', '>=', Carbon::now()->subMonths(7)->startOfDay())->orderBy('time', 'asc')->pluck('close')->toArray();
        $prices = [];
    ...
    {
        $models = Candle::where('tools_id', '=', $this->id)->where('tools_type', '=', 'coins')
            ->where('interval', '=', '1d')->where('time', '>=', Carbon::now()->subMonths(7)->startOfDay())->orderBy('time', 'asc')->get();
