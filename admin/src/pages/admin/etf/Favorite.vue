    ...
    <va-card>
        <va-card-content>
            ...
            <va-data-table :items="items" :columns="columns" :filter="filter" :filter-method="customFilteringFn"
                ...
                <template #cell(rsi_hour)="{ rowData }">
                    <va-chip size="small" :color="getCellRsiLabel(rowData.rsi_hour)">
                        {{ rowData.rsi_hour }}
                    </va-chip>
                </template>
            </va-data-table>
    ...
    methods: {
        ...
        unfavorite: function (event, rows) {
            ...
            axios.get("/sanctum/csrf-cookie").then((response) => {
                axios
                    .post("/api/etf/unfavorite", { selRows: self.selectedItemsEmitted })
                    .then((response) => {
                        if (response.status) {
